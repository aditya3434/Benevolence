...
String sql ="SELECT * FROM offers WHERE username LIKE '%"+session.getAttribute("search").toString()+"%' OR itemCategory LIKE '%"+session.getAttribute("search").toString()+"%'OR itemDesc LIKE '%"+session.getAttribute("search").toString()+"%' OR state LIKE '%"+session.getAttribute("search").toString()+"%' OR place LIKE '%"+session.getAttribute("search").toString()+"%'";
...
%>
  ...
  <h3><%=resultSet5.getString("itemCategory") %></h3><div class="ribbon"></div>
  <h4></h4>
  <%@ page import="java.sql.*"%>
  <%@ page import="java.io.*"%>
  <%@ page import="java.util.*"%>
  <%
  Blob blob = resultSet5.getBlob("image");
  ...
  <div class="address">
  <p>
  <%=resultSet5.getString("place") %>
  </p>
  </div>
  <div class="date">
  <p>
  <%=resultSet5.getString("username") %><br>
  <br>
  Date : <%=resultSet5.getDate("offerDate") %>
  </p>
  </div>
  <input type="hidden" name="id" value=<%=resultSet5.getString("itemID")%>>
  </div>
  </form>
<%
++i;
